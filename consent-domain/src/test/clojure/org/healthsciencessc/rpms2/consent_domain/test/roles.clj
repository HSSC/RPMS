...
(deftest test-role-checking
  ...
  (is (has-role? test-user {:code "sadmin"}) "Falsely reported a superadmin as invalid")
  (is (has-role? test-user {:code "sadmin"} :organization {:id 2})
    "Falsely reported a user with constraints as invalid")
  (is (not (has-role? test-user {:code "sadfmin"}))
      "User named sadfmin is not valid!")
  (is (not (has-role? test-user {:code "sadmin"} :organization {:id 2}
